...
def sampling(reaction_args):
    model = Model("sampling_model")
    reactions = []
    ...
    sampling_result = cobra.sampling.sample(model, reaction_args.number)
...
if __name__ == "__main__":
    ...
    try:
        ...
        for constrains_content in constrains_contents.split(args.constrains_content_splitter):
            ...
            sampling_result_values = sampling(args)
            flux_values_string = flux_values_string + "########### Constrains sample value start ###########" + "\n"
            for row in df_iter(sampling_result_values):
                flux_values_string = flux_values_string + str(row) + "\n"

        if len(flux_values_string) != 0:
            write_content_to_file(args.result_file, flux_values_string, "w")
    except Exception as e:
        execp = "Exception  occurs : " + str(e)
        write_content_to_file(args.result_file, execp, "w")
